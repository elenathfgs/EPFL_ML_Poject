...
def least_squares_SGD(y, tx, initial_w, batch_size, max_iters, gamma, loss_func):
...
def least_squares(y, tx, loss_func):
...
def ridge_regression(y, tx, loss_func, lambda_):
...
def logistic_regression(y, tx, initial_w, max_iters, gamma):
    ...
    for _ in range(max_iters):
        sig = expit(tx.dot(w)) # expit(x) = 1/(1+exp(-x))
        # calculate gradient and update w.
        grad = - np.dot(tx.T, 
                        (np.multiply(y, 1 - sig) - np.multiply(1 - y, sig)))
        w -= gamma * grad
        
        #calculate loss
        sig = expit(np.dot(tx, w))
        loss = - np.dot(y.T, np.log(sig)) - np.dot((1 - y.T), np.log(1 - sig))
    return w, loss
